...
def get_option_units(major, year):
    with driver.session() as session:
        ...
        RETURN u.unitcode as unitcode, u.unitname as unitname, u.type as type, u.semester as semester, u.major as major, u.level as level, u.credit_points as credit_points, u.points_req as points_req, or_req, and_req, unit_req, u.incompatible_units as incompatibilities, corequisites, u.notes as notes
        ORDER BY level
        """
        results = session.run(query)
        data = results.data()
